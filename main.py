        ...
        if kill_ever == 2:
            ...
            if under_level < 15:
                ...
                if message.text == user_name:
                    ...
                    data.change_command(user2_id, chat_id, 'Узгодження весілля', user_id)
            else:
                ...
                if message.text != user_name:
                    for j in range(5):
                        ...
                        if data.user_in_all_apartments_exist(user2_id, chat_id) != 0:
                            ...
                            data.change_command(user2_id, chat_id, 'Узгодження запрошення', user_id)
                else:
        ...
        if kill_ever == 2:
            ...
            for j in range(5):
                ...
                if user_id != user2_id:
                    ...
                    data.change_command(user2_id, chat_id, 'Узгодження переїзду', user_id)
                else:
                    data.change_apartment(user_id, chat_id, user_id)
                    await bot.send_message(chat_id, "Ви переїхали до себе")
        else:
...
async def do(message: types.Message):
    ...
    if message.chat.type in ['group', 'supergroup'] and data.user_exist(user_id, chat_id) == 1:
        ...
        if command != '':
            ...
            user2_id = data.get_data(user_id, chat_id, 'user2_id')
            user2_name = ''
            command_user2_id = data.get_data(user_id, chat_id, 'command_user2_id')
            if user2_id != 0:
                user2_name = data.get_data(user2_id, chat_id, 'name')
